...
void thread_init(void)
{
	...
	next_tick_to_wakeup = INT64_MAX; // 최솟값 찾아가야 하니 초기화할 때는 정수 최댓값. /* project 1 */
...
/*
   현재 스레드를 지정된 타이머 틱이 경과할 때까지 재웁니다.
...
// {
// 	enum intr_level old_level = intr_disable();
// 	struct list_elem *curr_elem = list_begin(&sleep_list); // sleep list의 첫번째 원소

// 	while (curr_elem != list_end(&sleep_list))
// 	{
// 		struct thread *curr_thread = list_entry(curr_elem, struct thread, elem); // 현재 검사중인 elem의 쓰레드

// 		if (curr_thread->wakeup_tick <= ticks) // 깨워야 할 시간인지 확인
// 		{
// 			curr_elem = list_remove(curr_elem); // sleep list 에서 제거
// 			thread_unblock(curr_thread);		// 스레드를 깨웁니다.
// 		}
// 		else
