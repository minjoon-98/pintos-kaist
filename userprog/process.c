...
tid_t process_create_initd(const char *file_name) {
  ...
  fn_copy =
      palloc_get_page(0);  // 페이지 할당 함수를 통해 메모리를 할당받습니다.
  if (fn_copy == NULL)  // 할당된 메모리가 없을 경우 에러를 반환
    return TID_ERROR;
  strlcpy(
      fn_copy, file_name,
      PGSIZE);  // file_name의 복사본을 fn_copy에 저장 (PGSIZE를 넘지 않도록 주의)

  // /* Extract the first token from FILE_NAME. */	 /* FILE_NAME에서 첫 번째 토큰을 추출합니다. */
  exec_name = strtok_r(
      file_name, " ",
      &save_ptr);  // 공백을 기준으로 첫 번째 토큰을 추출하여 save_ptr에 저장합니다.
  /* Create a new thread to execute FILE_NAME. */ /* FILE_NAME을 실행할 새로운 스레드를 생성합니다. */
  tid =
      thread_create(exec_name, PRI_DEFAULT, initd,
                    fn_copy);  // file_name을 실행할 새로운 스레드를 생성합니다.
...
tid_t process_fork(const char *name, struct intr_frame *if_ UNUSED) {
  ...
  memcpy(&parent->parent_if, if_,
         sizeof(struct intr_frame));  // 부모 프로세스 메모리를 복사

  tid_t child_tid =
      thread_create(name, PRI_DEFAULT, __do_fork,
                    parent);  // 전달받은 thread_name으로 __do_fork()를 진행
...
static bool duplicate_pte(uint64_t *pte, void *va, void *aux) {
  ...
  if (parent_page == NULL) {
    printf(
        "Virtual address(%llx) is not assigned in parent thread's page "
        "table.\n",
        va);
    return false;
  }
  ...
  newpage = palloc_get_page(
      PAL_USER |
      PAL_ZERO);  // PAL_USER 플래그 : 사용자 페이지를 할당, PAL_ZERO 플래그 : 할당된 페이지를 0으로 초기화
  if (newpage == NULL) {
...
static void __do_fork(void *aux) {
  ...
  sema_up(
      &current
           ->load_sema);  // Notify parent that fork is successful // 로드가 완료될 때까지 기다리고 있던 부모 대기 해제
...
int process_wait(tid_t child_tid UNUSED) {
  ...
  struct thread *child =
      get_child_process(child_tid);  // 자식 프로세스를 가져옵니다.
  if (child == NULL) {
...
void process_exit(void) {
  ...
  sema_up(
      &curr->wait_sema);  // 자식 스레드가 종료될 때 대기하고 있는 부모에게 signal을 보낸다. // 종료되었다고 기다리고 있는 부모 thread에게 signal 보냄-> sema_up에서 val을 올려줌

  // Wait for parent to acknowledge exit.
  sema_down(
      &curr->exit_sema);  // 자식 스레드가 완료되었음을 알리는 세마포어를 사용합니다. // 부모의 signal을 기다린다. 대기가 풀리고 나서 do_schedule(THREAD_DYING)이 이어져 다른 스레드가 실행된다. // 부모의 exit_Status가 정확히 전달되었는지 확인(wait)
}
...
bool lazy_load_segment(struct page *page, void *aux) {
  ...
  if (file_read_at(info->file, page->va, info->read_bytes, info->ofs) !=
      (int)info->read_bytes) {
...
static bool load_segment(struct file *file, off_t ofs, uint8_t *upage,
  ...
  while (read_bytes > 0 || zero_bytes > 0) {
    ...
    if (!vm_alloc_page_with_initializer(
            VM_ANON,
            upage,  // VM_ANON으로 초기화 해야함 -> 실행 파일은 변동 가능한 데이터를 다루기 때문에
            writable, lazy_load_segment,
            aux))  // VM_FILE은 .txt 파일과 같은 정적 파일들이 대상이다
    {
      free(aux);
      return false;
    }
