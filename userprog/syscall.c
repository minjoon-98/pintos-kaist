...
void syscall_handler(struct intr_frame *f UNUSED) {
  ...
  switch (syscall_number) {
    ...
    case SYS_MMAP:
      ...
      f->R.rax = mmap((void *)f->R.rdi, (size_t)f->R.rsi, (int)f->R.rdx,
                      (struct file *)f->R.r10, (off_t)f->R.r8);
...
void check_address(void *addr) {
  ...
  if (addr == NULL || !is_user_vaddr(addr) ||
      spt_find_page(spt, pg_round_down(addr)) == NULL) {
    // 잘못된 접근일 경우 프로세스 종료
    exit(-1);
  }
}
...
// 파일 객체를 검색하는 함수
struct file *get_file_from_fdt(int fd) {
  if (fd < 0 || MAX_FILES <= fd) return NULL;
...
void exit(int status) {
  ...
  printf(
      "%s: exit(%d)\n", curr->name,
      status);  // Process Termination Message /* 정상적으로 종료됐다면 status는 0 */
  thread_exit();
}
...
int exec(const char *cmd_line) {
  ...
  char *cl_copy;
  cl_copy =
      palloc_get_page(0);  // page를 할당받고 해당 page에 file_name을 저장해줌
  if (cl_copy == NULL) {
...
int filesize(int fd) {
  ...
  if (!f) return -1;  // 파일이 열려 있지 않은 경우 -1을 반환합니다.
...
static void check_buffer(uint64_t *buffer) {
  struct page *p = spt_find_page(&thread_current()->spt, buffer);
  if (!p) exit(-1);
  if (!p->writable && !p->original_writable) exit(-1);
}
...
int read(int fd, void *buffer, unsigned size) {
  ...
  if (buffer == NULL || is_kernel_vaddr(buffer) ||
      !spt_find_page(&thread_current()->spt, buffer))
    exit(-1);
  ...
  check_buffer((uint8_t *)buffer + size - 1);
  ...
  // struct file *f = thread_current()->fd_table[fd]; // 파일 디스크립터 테이블에서 파일 포인터를 가져옵니다.
    ...
    lock_acquire(
        &filesys_lock);  // file을 읽을 때 다른 프로세스의 접근을 막기 위해 lock
...
void *mmap(void *addr, size_t length, int writable, int fd, off_t offset) {
  ...
  if (addr == NULL || is_kernel_vaddr(addr) || (pg_round_down(addr) != addr) ||
      length >= KERN_BASE || (pg_round_down(offset) != offset))
    return NULL;
  ...
  // 이미 매핑된 주소 범위인지 확인
  if (check_address_overlap(addr, length)) return NULL;

  // 파일 디스크립터로부터 파일 객체 가져오기
  struct file *f = get_file_from_fdt(fd);
  if (length == 0 || f == NULL || file_length(f) == 0) exit(-1);
...
void munmap(void *addr) {
  ...
  if (!page || page->operations->type != VM_FILE) return;
  // lock_acquire(&filesys_lock); // ADD: filesys_lock at file system
